...
$parentModule = 'ResolveString'
if (-not $ModuleName) { $ModuleName = 'ResolveString.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ResolveString'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ResolveString.psd1"
$prereqHash = @{
